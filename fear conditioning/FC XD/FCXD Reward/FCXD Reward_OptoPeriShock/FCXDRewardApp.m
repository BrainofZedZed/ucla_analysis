classdef FCXDRewardApp < handle
    ...
    properties (Constant)
        AppPathBase = {'/FCXDReward'};
        AppClass = 'FCXDRewardApp';
        Increment = 1;
        Decrement = 0;
        Version = '13a';
    end
    ...
    methods
        ...
        function startApp(obj)
            ...
            mlock;
            FCXDRewardApp.refcount(obj.Increment);
            ...
            existVal = exist(fullfile(pwd,'FCXD_PGfork_Rewardfork')); %#ok<EXIST>
            ...
            if (doesShadowExist && ~pathOne.equals(pathTwo))
                % We are trying to execute the wrong FCXD_PGfork_Rewardfork
                errordlg(message('MATLAB:apps:runapp:WrongEntryPoint', 'FCXD_PGfork_Rewardfork').getString, ...
            ...
            if nargout(@FCXD_PGfork_Rewardfork) == 0
                eval('FCXD_PGfork_Rewardfork');
            else
                obj.AppHandle = eval('FCXD_PGfork_Rewardfork');
            end
