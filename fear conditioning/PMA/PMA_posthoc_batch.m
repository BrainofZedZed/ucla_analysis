...
tmp = Behavior.Intersect.SpaTemBeh.Freezing_platform_CSp_Vector;

tone_platform_frz = (sum(tmp,2)) ./ length(tmp);
tone_platform_frz = tone_platform_frz';

if P2.remove_baseline_tones
    tone_platform_frz = tone_platform_frz(P2.baseline_tones+1:end);
end

tone_platform_frz(isnan(tone_platform_frz)) = 0;
per_tpf = tone_platform_frz ./ tone_frz;
per_tpf = per_tpf*100;
per_tpf = per_tpf*100;  % need second 100 multiplier to make into percent
