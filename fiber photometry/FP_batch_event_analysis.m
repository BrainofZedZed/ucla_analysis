...
P2.baseline_per = [-10 0]; % baseline period relative to epoc onset for normalizing
...
for j = 1:length(P2.video_folder_list)
    ...
    clearvars -except 'P2' 'ct' 'auc_fc' 'auc_names_fc' 'peaks_shock_all' 'peaks_nonshock_all' 'peak_names' 'j' 'auc_shock_all' 'auc_nonshock_all' 'auc_shock_names' ' peaks_baseline_all' 'auc_baseline_all' 'peaks';
    % Initialize 
...
%% USER DEFINED BOUTS FOR ANALYSIS
   ...
   %%     
        ...
        if P2.do_auc
            ...
            auc_fc{ct,1} = exp_ID;
            auc_fc{ct,2} = trapz(meanz(t0:t0+(round(10*fp_fps))));

            try
                auc_fc{ct,3} = trapz(meanz(t0+round(10*fp_fps):t0+round(20*fp_fps)));
            catch
                auc_fc{ct,3} = '';
            end

            try
                auc_fc{ct,4} = trapz(meanz(t0+round(20*fp_fps):t0+round(30*fp_fps)));
            catch
                auc_fc{ct,4} = '';
            end

            try
                auc_fc{ct,5} = trapz(meanz(t0:t0+round(30*fp_fps)));
            catch
                auc_fc{ct,5} = '';
            end

            try
                auc_fc{ct,6} = trapz(meanz(t0+round(7*fp_fps):t0+round(12*fp+fps)));
            catch
                auc_fc{ct,6} = '';
            end
        ...
        if P2.save_analysis
          save(savename, 'data', 'bouts', 'bouts_name', 'zall', 'peaks', 'peak_names', 'auc_fc', 'auc_names_fc', 'sig', 'bhsig', 'P2', 'auc_shock', 'auc_shock_names', 'auc_nonshock', 'peaks_shock', 'peaks_nonshock', 'shock_trials', 'nonshock_trials', 'on_platform_shock', 'beh2fp', 'peaks_baseline', 'auc_baseline', 'zall_pf', 'zall_pf_avoid','zall_pf_nontone','zall_pf_tone');
        end
end
    ...
    if P2.save_analysis
        save(['fibpho_analysis_' bouts_name '.mat'], 'auc_fc','auc_names_fc','peaks', 'peaks_shock_all', 'peaks_nonshock_all', 'peak_names', 'Params', 'auc_shock_all', 'auc_shock_names', 'auc_nonshock_all', 'auc_baseline_all', 'peaks_baseline_all');
    end
