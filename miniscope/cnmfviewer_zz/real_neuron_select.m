...
%     tp = reshape(max(roi * sig, [], 2), pixh, pixw);
    ...
    for i = 1: nc
        ...
        clf
        pause(0.1)
        sgtitle(['Section #', num2str(i), '/', num2str(nc)])
...
%             plot(all_wave_lpf((i - 1) * nr + j, :), 'k')
        ...
        set(gcf, 'WindowButtonMotionFcn', @(object, eventdata) mouseMove(object, eventdata, tp, sig, rt, roi, sroi, hroi, hroi_wave, h, posf, posroi, i, nn, nr, pixh, pixw));
        set(gcf, 'WindowButtonDownFcn', @(object, eventdata) mouseClick(object, eventdata, tp, seeds, rt, roi, sroi, hroi, hsroi, h, posf, posroi, possroi, i, nn, nr, pixh, pixw));
