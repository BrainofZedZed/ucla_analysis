...
%   offsetIndex= ~floor(frameBuf + dayDur). Unused if you want a real measure from dayDurAll outside
...
function sorted_indices = orderByPeakTime(data, indices, tone_period)
    if isempty(indices)
        sorted_indices = [];
        return;
    end
    
    % Get peak times during tone period for the specified indices
    peak_times = zeros(length(indices), 1);
    for i = 1:length(indices)
        row_idx = indices(i);
        row_data = data(row_idx, tone_period);
        [~, max_idx] = max(row_data);
        peak_times(i) = max_idx;
    end
    
    % Sort by peak time
    [~, sort_idx] = sort(peak_times);
    sorted_indices = indices(sort_idx);
end

function orderedIDs = orderByPeak(periResp, neuronIDs, toneInds)
% orderByPeak:
%   Sort the given neuronIDs by the time of their maximum within periResp(...,toneInds).
%   If multiple maxima, 'max()' returns the first occurrence => earliest peak.
%   Returns a re-ordered vector of neuron IDs, earliest peak on top.
...
peakTimes = zeros(length(neuronIDs),1);
for k = 1:length(neuronIDs)
    nID = neuronIDs(k);
    rowVec = periResp(nID, toneInds);
    [~, loc] = max(rowVec);  
    peakTimes(k) = loc;
end

[~, sortIdx] = sort(peakTimes, 'ascend');
orderedIDs   = neuronIDs(sortIdx);
end
