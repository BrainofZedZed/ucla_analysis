function [locs_dist, frluX, frlu, objz, too_fast, locs] = ezlocTrack_v3_2TS(mscamnum, behavcamnum, dummybehavcamnum, frame_beh_sync, frame_ms_sync, frameStart, frameEnd, px2cm, fps_beh, fps_orig, fps)
...
%% working section on interpolating points that are too fast. Code runs but doesn't meaningfully reduce the problem points
disp('Done analyzing tracking data.');


% for i=1:length(too_fast)
%     x = locs_match((too_fast(i)-round(fps_beh/2):too_fast(i)-1),1);
%     x = [x; locs_match((too_fast(i)+1):too_fast(i)+round(fps_beh/2),1)];
%     [x1 index] = unique(x);
%     y = locs_match((too_fast(i)-round(fps_beh/2):too_fast(i)-1),2);
%     y = [y; locs_match((too_fast(i)+1):too_fast(i)+round(fps_beh/2),2)];
%     y1 = y(index);
% 
%     y_int = interp1(x1,y1,locs_match(too_fast(i),1)); 
%     
%     locs_match(too_fast(i),2) = y_int;
% end
%% plot movement
% figure;
% hold on
