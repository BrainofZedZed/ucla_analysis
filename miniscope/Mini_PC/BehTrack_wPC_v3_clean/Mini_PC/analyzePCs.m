function [PC_info, pc_ratemap, per_binoc] = analyzePCs (PC, spkmap, id, fps, gridprob, spkfn_fp, binsz)
...
% create and save rate map figs for all cells
for i = 1:length(pcs)
...
for i = 1:length(pcs)    
    ...
    [pf_angle, rho] = cart2pol(new_coords_x, new_coords_y);
...
% HARD CODE FOR ARENA SIZE (arenasz) AND SHAPE (binarea uses circle formula)
% NEEDS TO BE CHANGED IF USING DIFFERENT ARENA
arenasz = 50;    % size in cm
